 ...
 *  каждом вызове (а не один раз при загрузке истории), чтобы корректно обновляться при смене
 *  языка интерфейса без перезагрузки чата. Месяц переводится по текущей локали (это перевод
 *  названия, а не смена системы счисления времени, как у 24-часового формата выше). */
...
/** Стабильный, не зависящий от языка ключ календарного дня — для группировки сообщений по
 *  датам. Сравнивать переведённые метки (formatDateLabel) для группировки нельзя: у старых
 *  сообщений, загруженных до смены языка, и новых после — метки временно расходятся. */

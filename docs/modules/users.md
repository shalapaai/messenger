...
Поиск по email при этом ведёт себя по-разному в зависимости от наличия `"@"` в запросе:
- Если запрос уже содержит `"@"` (пользователь ввёл полный или частичный адрес вида `name@domain`) — сравнение идёт как обычная подстрока по всему email. Ограничивать поиск локальной частью здесь бессмысленно и даже вредно: паттерн `"%q%@%"` требовал бы **второго** `"@"` после найденного текста, которого в адресе с одним `"@"` не бывает — полный email вообще переставал бы находиться
- Если `"@"` в запросе нет — совпадение ищется только в локальной части (`vlad@gmail.com` → `"vlad"`), иначе даже одна буква совпадала бы почти со всеми пользователями одного почтового провайдера (`gmail.com`, `mail.ru` и т.п.), не отражая намерение найти конкретного человека

...
`LocalFileStorage` — единственная реализация `IFileStorage`, файлы сохраняются в папку на диске (`FileStorage:Local:BasePath`).
...
`fileKey` — GUID, генерируется заново на каждую загрузку и никогда не переиспользуется на другой контент: при замене аватарки старый файл удаляется, а не перезаписывается по тому же ключу (см. «Как работает загрузка аватарки» ниже). Поэтому ответ `GET /api/files/{fileKey}` можно считать иммутабельным и кэшировать надолго — эндпоинт ставит `Cache-Control: public, max-age=31536000, immutable` для аватарок и `private, ...` для вложений чата (доступ к ним всё равно проверяется по членству в чате, публичный кэш мог бы отдать файл не тому).

`Content-Length` выставляется явно из `record.SizeBytes` в БД, а не берётся из `Stream.Length` хранилища — так длина известна ещё до открытия файлового потока, а фронтенду она нужна, чтобы показать процент загрузки вложения (`onDownloadProgress` в `fileBlobCache.ts`).

## Ограничения

- Аватарки (личные и групповые): только изображения (JPEG/PNG/WebP/GIF), максимум **5 МБ**
- Вложения в сообщения: не любой тип — белый список (`AllowedAttachmentMimeTypes`): изображения, документы (PDF/Word/Excel/PowerPoint/txt/csv), архивы (zip/rar/7z), аудио, видео; исполняемые/скриптовые типы (exe, sh, bat, js и т.п.) намеренно исключены. SVG в списке изображений намеренно тоже нет — вложения отдаются напрямую по URL из origin хранилища, и встроенный `<script>` внутри SVG выполнился бы прямо там. Максимум **25 МБ** на файл, максимум **10** файлов на сообщение (см. [Messages](messages.md))

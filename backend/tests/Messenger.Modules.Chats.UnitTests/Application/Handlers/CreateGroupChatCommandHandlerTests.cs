...
public sealed class CreateGroupChatCommandHandlerTests
{
    ...
    {
        var command = new CreateGroupChatCommand(Guid.NewGuid(), "", [], null);
    ...
    public async Task Handle_ValidRequest_AddsCreatorAsOwner()
    {
        ...
        var command = new CreateGroupChatCommand(creatorId, "My Group", [], null);
        var result = await _sut.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_ValidRequest_AddsOtherMembersAsPlainMembers()
    {
        ...
        var command = new CreateGroupChatCommand(creatorId, "My Group", [memberId1, memberId2], null);
        var result = await _sut.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_CreatorIdIncludedInMemberIds_DoesNotAddDuplicateMembership()
    {
        ...
        var command = new CreateGroupChatCommand(creatorId, "My Group", [creatorId], null);
        var result = await _sut.Handle(command, CancellationToken.None);
    ...
    {
        var command = new CreateGroupChatCommand(Guid.NewGuid(), "My Group", [], null);

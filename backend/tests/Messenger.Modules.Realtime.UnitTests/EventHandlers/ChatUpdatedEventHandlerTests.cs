...
public sealed class ChatUpdatedEventHandlerTests
{
    ...
    public async Task Handle_SendsChatUpdatedToEachAffectedUsersPersonalGroup()
    {
        ...
        // 2 personal groups + 1 chat group, all resolved to the same mocked proxy
        await _clientProxy.Received(3).SendCoreAsync("ChatUpdated", Arg.Any<object?[]>(), Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_SendsChatUpdatedToChatGroup()
    {
        var chatId = Guid.NewGuid();

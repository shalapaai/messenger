...
public sealed class MessageEditedEventHandlerTests
{
    ...
    public async Task Handle_SendsToMembers_ViaFallback()
    {
        ...
        await _clientProxy.Received(2).SendCoreAsync("MessageEdited", Arg.Any<object?[]>(), Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_WhenMembersLookupFails_SendsNothing()
    {
        // MessageEdited больше не дублируется через chat:{id}-группу — единственный путь доставки
        ...
        await _clientProxy.DidNotReceive().SendCoreAsync(
            "MessageEdited", Arg.Any<object?[]>(), Arg.Any<CancellationToken>());
    }
}
